...
class TraceData:
    ...
    baseline_avg_latency: dict # average latencies for each video, on its nth regional re-download

    solution_latencies_per_region: dict
    solution_latencies_per_video: dict
...
for trial_idx, trace_name in enumerate(TRACE_NAMES):
    ...
    BASE_VM_FILES = os.listdir(f"{DATA_DIR}\\{trace_name}_baseline\\LF + SEQ (1.0)\\json")
    SOL_VM_FILES = os.listdir(f"{DATA_DIR}\\{trace_name}_sol\\GF + RND (1.0)\\json")
    ...
    for is_baseline, vm_files in [(True, BASE_VM_FILES), (False, SOL_VM_FILES)]:
        ...
        if is_baseline: latencies_per_region = data.baseline_latencies_per_region

        for vm_num, vm_data in enumerate([json.load(open(f"{DATA_DIR}\\{trace_name}_baseline\\LF + SEQ (1.0)\\json\\{vm_file}", 'r')) for vm_file in vm_files], start = 2):
            vmname = f"vm{vm_num}"
...
for trial_idx, trace_data in enumerate(all_data):
    ...
    N_DOWNLOADS = 4
    N_VIDEOS = json.load(open(f"{EVENTS_DIR}\\{trace_name}_workload\\events.json", 'r'))["total_files_generated"]


    for is_baseline, latencies_per_region in [(True, trace_data.baseline_regional_latencies), (False, trace_data.solution_regional_latencies)]:
    
        # latencies_per_video = np.zeros((N_VIDEOS, N_DOWNLOADS))
        ...
        avg_latency = np.zeros((1, N_DOWNLOADS))
        stdev_latency = np.zeros((1, N_DOWNLOADS))
        ...
        avg_latency = np.average(latencies_per_video, axis=1) # Average over all videos, on their nth re-download -> Should get row array of size 4

        for i, (num, denom) in enumerate(zip (sum_unique_download_latencies, num_unique_downloads)):
            avg_latency[i] = num/denom

        # TODO: Calculate standard deviation. this way also
        stdev_latency = np.std(latencies_per_video, axis=1) # Get the accompanying stdev
